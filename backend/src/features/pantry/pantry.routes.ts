...
import { freshnessFromText } from './freshness.service.js';
...
pantryRouter.get('/', requireAuth, async (request, response, next) => {
  try {
    ...
    user.pantryItems.forEach((item) => {
      if (item.freshness.bestByDate || !item.extractedText) return;
      const { freshness } = freshnessFromText(item.extractedText, item.name, item.storageMethod, item.purchasedAt);
      if (freshness.bestByDate) { item.set('freshness', freshness); updated = true; }
    });
    if (updated) await user.save();
    response.json({ pantryItems: user.pantryItems });
  }
